...
import { formatString, getWeekNumber } from '../utils/utils';
...
type WorkingDayListObject = WorkingDaySchedule | WorkingDayScheduleWeekDivider | 'space';

export const usePultosokAppDisplayData = (workingDays: WorkingDaySchedule[] | undefined) => {
  // create a new array based on the working days,
  // but add the week dividers and spaces
  const workingDaysWithDividers: WorkingDayListObject[] | undefined = useMemo(() => {
...
export const WorkingDayListObject = ({ item }: { item: WorkingDayListObject }) => {
  const { l } = useLocale();
