...
export const useApkUpdaterContextHook = () => {
  const { latestApkVersion, isUpdateAvailable, isUpdateAvailableChecked } = useApkUpdateChecker();
  const { isDownloadCompleted, downloadLatestApk, downloadPercent, downloadError, latestApkDownloadPath } =
    useApkDownloader();
  ...
  useEffect(() => {
    ...
    console.error('Error occured while downloading the update: ' + downloadError);
  }, [downloadError]);
